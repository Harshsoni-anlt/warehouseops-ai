...
export const equipmentAPI = {
  ...
  createAsset: async (data: Partial<EquipmentAsset>): Promise<any> => {
  ...
  updateAsset: async ({ asset_id, updates }: { asset_id: string; updates: Partial<EquipmentAsset> }): Promise<any> => {
    const safeId = validatePathParam(asset_id, 'asset_id');
    const response = await api.put(`/equipment/${safeId}`, updates);
    return response.data;
  },
