    ...
    try {
      ...
      if (response.document_id) {
        ...
        const newDocument: DocumentItem = {
          ...
          progress: 0,
          stages: [
            { name: 'Preprocessing', completed: false, current: true, description: 'Reading the file and extracting raw text' },
            { name: 'OCR Extraction', completed: false, current: false, description: 'Text extraction (pdfplumber)' },
            { name: 'LLM Processing', completed: false, current: false, description: 'LLM pulls structured fields (Groq)' },
  ...
  const ProcessingPipelineCard = () => (
    ...
    >
      <CardContent>
        ...
        <List dense>
          {[
            { name: '1. Upload', description: 'PDF or text file is stored securely', color: 'primary' },
            { name: '2. Text extraction', description: 'pdfplumber reads the document text', color: 'primary' },
            { name: '3. LLM extraction', description: 'Groq pulls vendor, dates, totals, line items', color: 'primary' },
            { name: '4. Indexing', description: 'Local embeddings stored in ChromaDB for search', color: 'primary' },
            { name: '5. Quality check', description: 'Extraction scored for completeness', color: 'primary' },
            { name: '6. Routing', description: 'Auto-approve or flag for human review', color: 'primary' },
          ].map((stage, index) => (
      ...
      <CardContent>
        ...
        <Box
          sx={{
            ...
            '&:hover': {
              borderColor: isUploading ? '#E6E9EF' : '#0D9488',
              backgroundColor: isUploading ? '#F6F8FB' : 'rgba(13, 148, 136, 0.05)',
            },
          }}
          onClick={() => {
        ...
        >
          <Typography variant="body2" sx={{ fontSize: '0.875rem' }}>
            Documents are processed locally with pdfplumber and a Groq LLM for extraction, 
      ...
      <CardContent>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          ...
          <Chip 
            ...
            size="small"
            sx={{
              backgroundColor: document.status === 'completed' ? '#3FB950' : '#0D9488',
              color: '#000000',
              fontWeight: 600,
              fontSize: '0.75rem',
            }}
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Typography 
                      ...
                      sx={{ 
                        ...
                        color: stage.completed ? '#3FB950' : stage.current ? '#0D9488' : '#64748B',
                        fontSize: '0.875rem',
                      }}
                    >
                      {stage.name}
  ...
  return (
    <Box sx={{ p: 0 }}>
      <Box sx={{ mb: 4 }}>
        ...
        >
          Upload warehouse documents for intelligent extraction and processing using local models
        </Typography>
      </Box>

      <Box sx={{ borderBottom: '1px solid #E6E9EF', mb: 3 }}>
        <Tabs 
          value={activeTab} 
                ...
                <CardContent>
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      >
                        <strong style={{ color: '#0F172A' }}>Quality Score:</strong>{' '}
                        <Chip 
                          label={`${documentResults.quality_score}/5.0`}
                          size="small"
                          sx={{ 
                            ml: 1,
                            backgroundColor: documentResults.quality_score >= 4 ? '#3FB950' : documentResults.quality_score >= 3 ? '#0D9488' : '#D29922',
                            color: '#000000',
                            fontWeight: 600,
                            fontSize: '0.75rem',
                          }}
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      >
                        <strong style={{ color: '#0F172A' }}>Routing Decision:</strong>{' '}
                        <Chip 
                          label={documentResults.routing_decision}
                          size="small"
                          sx={{ 
                            ml: 1,
                            backgroundColor: documentResults.routing_decision === 'auto_approve' ? '#3FB950' : documentResults.routing_decision === 'flag_review' ? '#0D9488' : '#F85149',
                            color: '#000000',
                            fontWeight: 600,
                            fontSize: '0.75rem',
                          }}
                              ...
                              {Object.entries(combinedInfo).map(([key, value]) => (
                                <Grid item xs={12} sm={6} key={key}>
                                  ...
                                  >
                                    ...
                                    <span style={{ color: '#64748B' }}>{String(value)}</span>
                                  </Typography>
                                </Grid>
                              ))}
                            </Grid>
