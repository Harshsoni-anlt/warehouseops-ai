...
const EquipmentNew: React.FC = () => {
  ...
  const createAssetMutation = useMutation({
    mutationFn: equipmentAPI.createAsset,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['equipment'] });
      setSaveError(null);
      handleClose();
    },
    onError: (err: any) => setSaveError(err?.response?.data?.message || 'Failed to create asset'),
  });

  const updateAssetMutation = useMutation({
    mutationFn: equipmentAPI.updateAsset,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['equipment'] });
      setSaveError(null);
      handleClose();
    },
    onError: (err: any) => setSaveError(err?.response?.data?.message || 'Failed to update asset'),
  });

  const handleSaveAsset = () => {
    setSaveError(null);
    if (!formData.asset_id || !formData.type) {
      setSaveError('Asset ID and Type are required');
      return;
    }
    if (selectedAsset) {
    ...
    <Box sx={{ width: '100%', maxWidth: 'none', p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Button
          ...
          startIcon={<AddIcon />}
          onClick={() => handleOpen()}
          sx={{ backgroundColor: '#0D9488', '&:hover': { backgroundColor: '#5a8f00' } }}
        >
          Add Asset
        </Button>
      </Box>
