    ...
    try:
        ...
        if text:
            try:
                ...
                prompt = (
                    "You are a warehouse document extraction assistant. From the document text "
                    "below, return ONLY a compact JSON object with keys: document_type, vendor, "
                    "document_date, total_amount, line_items (array of {description, quantity, "
                ...
                logger.warning(f"LLM extraction fell back for {document_id}: {e}")
                summary = text[:200]
