...
class AdvancedForecastingService:
    ...
    async def initialize(self):
        ...
        try:
            # SQLite connection (asyncpg-compatible facade)
            from src.api.services.database import get_database_connection
            self.pg_conn = await get_database_connection()

            # Set db_pool to pg_conn for compatibility with model performance methods
            self.db_pool = self.pg_conn
            
            # Optional Redis cache. Synchronous redis calls block the event
            # loop, so only enable it if a server actually responds; otherwise
            # run without caching (forecasts are cheap to recompute).
            self.redis_client = None
            try:
                ...
                logger.info("✅ Redis cache connected for forecasting")
            except Exception:
                logger.info("Redis not available; forecasting runs without caching")
    ...
    async def get_real_time_forecast(self, sku: str, horizon_days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            # Cache the result for 1 hour (only if Redis is available)
            if self.redis_client:
                try:
    ...
    async def get_model_performance_metrics(self) -> List[ModelPerformanceMetrics]:
        """Get model performance metrics and drift detection (cached ~60s to
        avoid recomputing the same ~36 queries on every caller)."""
        import time as _t
        _now = _t.time()
        cached = getattr(self, "_mpm_cache", None)
        if cached and (_now - cached[0]) < 60:
            return cached[1]
