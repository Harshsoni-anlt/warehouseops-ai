...
async def create_equipment_asset(asset: EquipmentAssetCreate):
    """Create (or upsert) an equipment asset."""
    try:
        await sql_retriever.initialize()
        query = """
            INSERT INTO equipment_assets
                (asset_id, type, model, zone, status, owner_user, next_pm_due, created_at, updated_at)
            ...
            ON CONFLICT(asset_id) DO UPDATE SET
                ...
                next_pm_due = excluded.next_pm_due, updated_at = NOW()
        """
        await sql_retriever.execute_command(
            query, asset.asset_id, asset.type, asset.model, asset.zone,
            asset.status, asset.owner_user, asset.next_pm_due,
        )
        return {"success": True, "asset_id": asset.asset_id, "message": "Equipment asset saved"}
    except Exception as e:
        logger.error(f"Failed to create equipment asset: {e}")
        raise HTTPException(status_code=500, detail="Failed to create equipment asset")


@router.put("/equipment/{asset_id}", response_model=Dict[str, Any])
async def update_equipment_asset(asset_id: str, updates: Dict[str, Any] = Body(...)):
    """Update fields on an equipment asset. Body: any of type/model/zone/status/owner_user/next_pm_due/last_maintenance."""
    try:
        await sql_retriever.initialize()
        allowed = {"type", "model", "zone", "status", "owner_user", "next_pm_due", "last_maintenance"}
        fields = {k: v for k, v in (updates or {}).items() if k in allowed}
        if not fields:
            raise HTTPException(status_code=400, detail="No valid fields to update")
        set_clause = ", ".join(f"{k} = ${i + 1}" for i, k in enumerate(fields))
        params = list(fields.values()) + [asset_id]
        query = f"UPDATE equipment_assets SET {set_clause}, updated_at = NOW() WHERE asset_id = ${len(fields) + 1}"
        await sql_retriever.execute_command(query, *params)
        return {"success": True, "asset_id": asset_id, "updated_fields": list(fields.keys())}
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Failed to update equipment asset: {e}")
        raise HTTPException(status_code=500, detail="Failed to update equipment asset")
