...
warehouse documentation and operational procedures using the LLM service.
"""
...
class EmbeddingService:
    ...
    Uses a local sentence-transformers model (all-MiniLM-L6-v2, 384-dim) by
    default, delegating to the shared LLM client's local embedder.
    """
    ...
    async def initialize(self) -> None:
        """Initialize the embedding service with the LLM client."""
        try:
            ...
            logger.info(f"Embedding service initialized with embedding model: {self.model_name}")
    ...
    async def generate_embedding(self, text: str, input_type: str = "query") -> List[float]:
        """
        Generate embedding for a single text using the LLM service.
    ...
    async def generate_embeddings(self, texts: List[str], input_type: str = "query") -> List[List[float]]:
        """
        Generate embeddings for multiple texts using the LLM service.
        ...
        try:
            ...
            logger.info(f"Generated {len(response.embeddings)} embeddings using the LLM service")
